...
export default function FileViewer({ files, emptyMessage = "No output available." }: FileViewerProps) {
  const { toast } = useToast();
  const [copiedFileId, setCopiedFileId] = useState<string | null>(null);
  const defaultTab = useMemo(() => files[0]?.id ?? "", [files]);

  const handleCopy = async (file: FilePreview) => {
    try {
      ...
      {files.map((file) => (
        <TabsContent key={file.id} value={file.id}>
          <div className="flex items-center justify-between">
            <p className="text-xs text-grey-500">CSV output</p>
            <Button
              type="button"
              variant="outline"
              size="sm"
